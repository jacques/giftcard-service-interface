...
public abstract class ActivationsResource {
   ...
   public final void confirmActivation(
      ...
      asyncResponse.resume(getResourceImplementation().confirmActivation(
            activationId,
            confirmationId,
   ...
   @ApiOperation(value = "Request a gift card activation.", notes = "The Activations endpoint allows a "
         ...
         + "activation request should only be sent once.", authorizations = { @Authorization(value = "httpBasic") }, tags = { "Activations", })
   @ApiResponses(value = {
         @ApiResponse(code = 201, message = "Created", response = ActivationResponse.class, responseHeaders = { @ResponseHeader(name = "Location", description = "The location of the created activation resource", response = String.class) }),
         @ApiResponse(code = 400, message = "Bad Request", response = ErrorDetail.class),
         @ApiResponse(code = 500, message = "Internal Server Error", response = ErrorDetail.class),
         @ApiResponse(code = 503, message = "Service Unavailable", response = ErrorDetail.class),
         @ApiResponse(code = 504, message = "Gateway Timeout", response = ErrorDetail.class) })
   public final void activate(
         ...
         @Context UriInfo uriInfo,
         @Context HttpServletRequest httpServletRequest) {
      asyncResponse.resume(getResourceImplementation().activate(
            activationId,
            activationRequest,
            securityContext,
            httpHeaders,
   ...
   public final void reverseActivation(
         ...
         @Context UriInfo uriInfo,
         @Context HttpServletRequest httpServletRequest) {
      asyncResponse.resume(getResourceImplementation().reverseActivation(
            activationId,
            reversalId,
            activationReversal,
            securityContext,
